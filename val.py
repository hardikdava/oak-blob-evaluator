...
from utils.general import (LOGGER, TQDM_BAR_FORMAT, Profile, check_dataset, check_img_size, check_requirements,
                           ...
                           print_args, scale_boxes, xywh2xyxy)
...
def run(
    ...
    # Load model
    model = DetectMultiBackend(weights, device=device, oak=True)
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # NMS
        targets[:, 2:] *= torch.tensor((width, height, width, height), device=device)  # to pixel
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'v7_blob', help='model path(s)')
